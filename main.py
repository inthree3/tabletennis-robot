...
while True:
    tm.start()
    src_0 = cv2.imread('cam0_ex.jpg')
    src_0 = cv2.remap(src_0, mapx0, mapy0, cv2.INTER_LINEAR)
    src_0 = cv2.copyTo(src_0, mask0)

    src_1 = cv2.imread('cam1_ex.jpg')
    src_1 = cv2.remap(src_1, mapx1, mapy1, cv2.INTER_LINEAR)
    ...
    # TriangulatePoints --> Epipolar Geometry 이용하여 공의 3차원 위치 인식하는 코드
